...
It is recommended that users use the internal prometheus service address, which is always the same for every cluster and
prevents this traffic to go out and in the cluster. (`https://prometheus-k8s.openshift-monitoring.svc.cluster.local:9091`)

Alternatively, in order to retrieve the external prometheus service address users can do so by executing
